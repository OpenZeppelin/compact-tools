...
- **Breaking:** `createLiveContext` takes midnight-js types. `findOptions` replaces `contractAddress`, `compiledContract` and `privateStateId`. `providersFor` returns `ContractProviders`, and `providersFor(null)` replaces `publicDataProvider` and `privateStateProvider`. `@midnight-ntwrk/compact-js` joins the optional peers
