...
- [`packages/lint`](./packages/lint) — bin wrapper around the [`compact-lint`](./crates/compact-lint) crate, the doc-comment linter for Compact sources
...
- Node.js >= 22 for `packages/builder` and `packages/simulator`, >= 24 for `packages/cli`, `packages/deployer` and `packages/lint`
- Yarn 4 (Berry)
- Turbo
