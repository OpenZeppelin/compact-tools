...
7. Once published, go to "Releases" and create a GitHub release using the
   generated tag. Mark beta tags as prereleases. `compact-lint` is the
...
compact-lint (bin wrapper around the compact-lint crate)
...
4. `compact-lint` (no internal deps)
...
`compact-lint` is a bin wrapper around the `compact-lint` crate, so its release
carries a native binary as well as an npm package.

- The version lives in two files: `packages/lint/package.json` and
  ...
  test in `packages/lint` fails when the two drift.
...
- It then creates the GitHub release `compact-lint/v<version>` with the four assets
...
- Release notes come from the `## <version>` section of `packages/lint/CHANGELOG.md`,
  falling back to the tag name. A `beta` dist-tag marks the release a prerelease.
- Before the first release, add `@openzeppelin/compact-lint` as a Trusted Publisher
