 ...
 *
 * This is the property `compact-deploy`'s local resolution depends on: if
 * it breaks (e.g. a new node release changes the prefunded set), every
 * other spec will start failing with InsufficientFunds.
 *
 * Owns its own pool. Stopping the shared one here would leave every spec
 * that runs after this file to re-sync its wallets from genesis.
 */
describe('compact-deploy — prefunded wallet pool', () => {
  ...
  beforeAll(() => {
    setupLocalNetwork();
    pool = new WalletPool(localNetworkConfig());
  });
