...
export const PRIVATE_COUNTER_ARTIFACT_DIR = resolve(
  INTEGRATION_DIR,
...
/**
 * Same hint shape as {@link requireFixtureArtifact} but for the
 * `PrivateCounter` fixture used by the private-state and
 * witnesses-module specs.
 */
export function requirePrivateCounterArtifact(): void {
  if (existsSync(PRIVATE_COUNTER_ARTIFACT_DIR)) return;
  throw new Error(
    `Missing compiled artifact at ${PRIVATE_COUNTER_ARTIFACT_DIR}.\n` +
      'Run `make -C tests/integrations compile` first.',
  );
}
